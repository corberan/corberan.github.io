...
Module['FS_createPath']("/", "fonts", true, true);
    ...
    loadPackage({"files": [{"filename": "/fonts/simsun.ttf", "start": 0, "end": 15313900}], "remote_package_size": 15313900});

  })();

// end include: /tmp/tmpom9etypg.js
// include: /tmp/tmp8vailukl.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmp8vailukl.js
// include: /tmp/tmpi_81sml6.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmpi_81sml6.js
...
// Begin JS library code
  ...
  instrumentWasmImports(imports) {
        var importPattern = /^(sync_filesystem|invoke_.*|__asyncjs__.*)$/;
...
var ASM_CONSTS = {
 ...
 523628: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 524616: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 524699: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 524768: () => { return window.innerWidth; },  
 524798: () => { return window.innerHeight; }
};
function __asyncjs__sync_filesystem() { return Asyncify.handleAsync(async () => { await new Promise((resolve, reject) => { FS.syncfs(true, (err) => { if (err) reject(err); else resolve(); }); }); }); }
...
function assignWasmExports(wasmExports) {
  ...
  assert(wasmExports['dynCall_iiiiiiii'], 'missing Wasm export: dynCall_iiiiiiii');
  dynCall_iiiiiiii = dynCalls['iiiiiiii'] = createExportWrapper('dynCall_iiiiiiii', 8);
  assert(wasmExports['dynCall_iiiiiiiiii'], 'missing Wasm export: dynCall_iiiiiiiiii');
  dynCall_iiiiiiiiii = dynCalls['iiiiiiiiii'] = createExportWrapper('dynCall_iiiiiiiiii', 10);
  assert(wasmExports['dynCall_iiiiiiiiiiiiiiff'], 'missing Wasm export: dynCall_iiiiiiiiiiiiiiff');
  dynCall_iiiiiiiiiiiiiiff = dynCalls['iiiiiiiiiiiiiiff'] = createExportWrapper('dynCall_iiiiiiiiiiiiiiff', 16);
  assert(wasmExports['dynCall_iiiiiiiii'], 'missing Wasm export: dynCall_iiiiiiiii');
  ...
  assert(wasmExports['dynCall_viiiiiiiiiii'], 'missing Wasm export: dynCall_viiiiiiiiiii');
  dynCall_viiiiiiiiiii = dynCalls['viiiiiiiiiii'] = createExportWrapper('dynCall_viiiiiiiiiii', 12);
  assert(wasmExports['dynCall_iiiiiidiiff'], 'missing Wasm export: dynCall_iiiiiidiiff');
  ...
  assert(wasmExports['dynCall_iiiiid'], 'missing Wasm export: dynCall_iiiiid');
